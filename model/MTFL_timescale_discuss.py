...
def final_train_CNN_for_OPPV(epochs=8, train_batch_size=16):
    ...
    clf = LogisticRegression(penalty='l2', solver='lbfgs').fit(feat_in, label)
    return clf
...
if __name__ == '__main__':
    ...
    # 测试第一个模型
    clf1 = final_train_CNN_for_OPPV()
    with open('/home/zhangkexin/code/work1/finalresult/mtfl_ts/5/clf1.pickle', 'wb') as f:
        pickle.dump(clf1, f)
    with open('/home/zhangkexin/code/work1/finalresult/mtfl_ts/5/clf1.pickle', 'rb') as f:
        clf1_reload = pickle.load(f)
    pred_1, real_1, loopname1 = final_test_CNN_for_OPPV(clf1_reload)

    # 测试第二个模型
    clf2 = final_train_CNN_for_OPPV_dist()
    with open('/home/zhangkexin/code/work1/finalresult/mtfl_ts/5/clf2.pickle', 'wb') as f:
        pickle.dump(clf2, f)
    with open('/home/zhangkexin/code/work1/finalresult/mtfl_ts/5/clf2.pickle', 'rb') as f:
    ...
    featnew = np.concatenate((feat1[:,0:NNN], feat1[:,0:84], feat2[:, 0:NNN],feat2[:, 0:84] ), axis=1)
    print(featnew.shape)
    clf = LogisticRegression(penalty = 'l2', solver = 'newton-cg').fit(featnew, labels)
    with open('/home/zhangkexin/code/work1/finalresult/mtfl_ts/5/clf.pickle', 'wb') as f:
        pickle.dump(clf, f)
    with open('/home/zhangkexin/code/work1/finalresult/mtfl_ts/5/clf.pickle', 'rb') as f:
        clf_reload = pickle.load(f)
    f, l = get_test_feats_fusion()
    feat1t = f[:, 0:336]
    feat2t = f[:, 336:672]
    print(feat1t.shape, feat2t.shape)
    featnewt = np.concatenate((feat1t[:, 0:NNN],feat1t[:, 0:84], feat2t[:, 0:NNN], feat2t[:, 0:84]), axis=1)
    final_pred = clf_reload.predict(featnewt)
    print('-------------------------------------------------------')
